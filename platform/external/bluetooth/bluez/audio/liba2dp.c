		...
		const bt_audio_msg_header_t *msg)
{
	...
	INFO("sending %s %s", bt_audio_strtype(msg->type), 
		...
		bt_audio_msg_header_t *inmsg)
{
	...
	INFO("trying to receive msg from audio service... fd: %d, lenght: %d", 
                                                                    data->server.fd, length);
...
{
	INFO("expected_name %d", expected_name);
...
int a2dp_init(int rate, int channels, a2dpData* dataPtr)
{
	...
	INFO("a2dp_init rate: %d channels: %d", rate, channels);
	*dataPtr = NULL;
