...
static gboolean stream_open_timeout(gpointer user_data)
{
	...
	error("%s: Timed out waiting for peer to open the transport channel", __FUNCTION__);
...
static void stream_free(struct avdtp_stream *stream)
{
    ...
    if (stream == NULL){
    	error("stream already has been freed, just return");
    	return;
    }
	stream->lsep->info.inuse = 0;
	stream->lsep->stream = NULL;
	
    if (stream->session->seps == NULL){
    	error("stream already has been freed, just return");
    	return;
    }
...
static void finalize_discovery(struct avdtp *session, int err)
{
	...
	if (!session->discov_cb)
		return;
	if (!session->seps){
		error("finalize_discovery session->seps is null");
		return;
	}
...
static void avdtp_connect_cb(GIOChannel *chan, GError *err, gpointer user_data)
{
	...
	ba2str(&session->dst, address);
	INFO("AVDTP: connected %s channel to %s",
...
int avdtp_start(struct avdtp *session, struct avdtp_stream *stream)
{
	...
	INFO("");
	if (!g_slist_find(session->streams, stream)) {
		error("avdtp_start: Finds the element in a stream fail");
		return -EINVAL;
	}

	if (stream->lsep->state != AVDTP_STATE_OPEN) {
		error("avdtp_start: state is not OPEN");
		return -EINVAL;
	}
	...
	if (stream->starting == TRUE) {
		error("avdtp_start: stream already started");
		return -EAGAIN;
	}
