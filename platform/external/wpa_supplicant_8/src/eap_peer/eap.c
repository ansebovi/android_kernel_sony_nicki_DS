...
#include "gsm_domain.h"  //CONN-FY-FP7372_EAPSIM_Integration-00
...
#if defined(PCSC_FUNCS) || defined(PCSC_FUNCS_SEMC)  //CONN-FY-FP7372_EAPSIM_Integration-00
...
static char * get_gsm_domain(struct eap_sm *sm,
			     const char *mcc_str, const char *mnc_str)
{
	int mcc, mnc;
	struct wpa_gsm_domain *domain;

	mcc = atoi(mcc_str);
	mnc = atoi(mnc_str);

	domain = sm->gsm_domain;
	while (domain) {
		if (domain->mcc == mcc && domain->mnc == mnc)
...
{
	int mnc_len, realm_len;
	char mcc[4], mnc[4], *domain, realm[100];

	/* MCC (3 digits) */
	os_memcpy(mcc, imsi, 3);
	mcc[3] = '\0';
	...
	domain = get_gsm_domain(sm, mcc, mnc);
	if (domain != NULL) {
		wpa_printf(MSG_INFO, "GSM domain is changed to: %s", domain);
		realm_len = os_snprintf(realm, sizeof(realm), "@%s", domain);
	} else {
		wpa_printf(MSG_INFO, "Using default home network domain: "
			   "wlan.mncXXX.mccXXX.3gppnetwork.org");
		realm_len = os_snprintf(realm, sizeof(realm),
					"@wlan.mnc%s.mcc%s.3gppnetwork.org",
					mnc, mcc);
	}

	if (realm_len < 0 || (size_t) realm_len >= sizeof(realm) ||
	    (*imsi_len + realm_len > max_len)) {
		wpa_printf(MSG_WARNING, "No room for realm in IMSI buffer");
		return -1;
	}

	os_memcpy(imsi + *imsi_len, realm, realm_len);
	*imsi_len += realm_len;
...
#endif /* PCSC_FUNCS || PCSC_FUNCS_SEMC */
				...
				struct eap_peer_config *conf)
{
	...
	return 0;
#else /* PCSC_FUNCS || PCSC_FUNCS_SEMC */
	return -1;
#endif /* PCSC_FUNCS || PCSC_FUNCS_SEMC */
}
				 ...
				 void *msg_ctx, struct eap_config *conf)
{
	...
	sm->ClientTimeout = EAP_CLIENT_TIMEOUT_DEFAULT;
	sm->wps = conf->wps;
	sm->gsm_domain = conf->gsm_domain;   //CONN-FY-FP7372_EAPSIM_Integration-00
