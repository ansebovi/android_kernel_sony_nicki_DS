...
static const struct parse_data ssid_fields[] = {
	...
	{ STRe(pcsc) },
	{ STR_KEYe(pin) },
	{ INTe(sim_slot) },    //CONN-FY-DS_Support_EAPSIM
	{ STRe(engine_id) },
	{ STRe(key_id) },
	{ STRe(cert_id) },
	{ STRe(ca_cert_id) },
...
void wpa_config_free(struct wpa_config *config)
{
	...
	struct wpa_gsm_domain *domain, *prevdomain = NULL;   //CONN-FY-FP7372_EAPSIM_Integration-00
...
#endif /* CONFIG_NO_CONFIG_BLOBS */
//CONN-FY-FP7372_EAPSIM_Integration-00++
	domain = config->gsm_domain;
	while (domain) {
		prevdomain = domain;
		domain = domain->next;
		os_free(prevdomain->domain);
		os_free(prevdomain);
	}    
	os_free(config->gsm_domain_file);
	//CONN-FY-FP7372_EAPSIM_Integration-00--
...
//CONN-FY-FP7372_EAPSIM_Integration-00++
static int wpa_config_add_gsm_domain(struct wpa_config *config,
	int mcc, int mnc, char *domain)
{
	struct wpa_gsm_domain *gsm_domain, *last = NULL;

	gsm_domain = config->gsm_domain;
	while (gsm_domain) {
		last = gsm_domain;
		gsm_domain = gsm_domain->next;
	}

	gsm_domain = os_zalloc(sizeof(*gsm_domain));
	if (gsm_domain == NULL)
		return -1;
	if (last)
		last->next = gsm_domain;
	else
		config->gsm_domain = gsm_domain;

	gsm_domain->mcc = mcc;
	gsm_domain->mnc = mnc;
	gsm_domain->domain = domain;

	wpa_printf(MSG_DEBUG, "GSM home network domain added: %03d %03d %s",
		   mcc, mnc, domain);
...
/**
 * wpa_config_process_gsm_domain_file - Process home network domain conf. file
 *
 * Parse home network domain configuration file.
 * Configuration file has following format:
 * # Comment
 * MCC MNC Domain
 */
static int wpa_config_process_gsm_domain_file(
	const struct global_parse_data *data,
	struct wpa_config *config, int line, const char *pos)
{
	FILE *f;
	char buf[256], *ptr, *token, *domain = NULL;
	int n, mcc = 0, mnc = 0;

	if (!pos[0]) {
		wpa_printf(MSG_ERROR, "Line %d: invalid gsm_domain_file", line);
		return -1;
	}

	config->gsm_domain_file = os_strdup(pos);
	if (config->gsm_domain_file == NULL)
		return -1;

	wpa_printf(MSG_INFO, "Reading GSM home network domain configuration "
		   "file '%s'", config->gsm_domain_file);

	f = fopen(config->gsm_domain_file, "r");
	if (f == NULL) {
		wpa_printf(MSG_ERROR, "Could not open %s file: %s",
			config->gsm_domain_file, strerror(errno));
		return -1;
	}
	...
	while (fgets(buf, sizeof(buf), f) != NULL) {
		...
		while (token && n < 3) {
			if (!token) {
				wpa_printf(MSG_ERROR,
					   "Line %d: invalid GSM home network "
					   "domain record", line);
				goto error_exit;
			}
		...
		if (n < 3) {
			wpa_printf(MSG_ERROR,
				   "Line %d: bad format of GSM home network "
				   "domain record, skipping: '%s'", line, buf);
		} else
		if (wpa_config_add_gsm_domain(config, mcc, mnc, domain)) {
			wpa_printf(MSG_ERROR,
				   "Line %d: failed to add GSM home network "
				   "domain record", line);
			goto error_exit;
		}
