...
static int wpa_supplicant_ctrl_iface_set_apdu_socket(
	struct wpa_supplicant *wpa_s, char *apdu_socket)
{
	size_t len;
	int ret;
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0 ||
		...
		wpa_hexdump_ascii(level, "RX ctrl_iface",
				  (const u8 *) buf, os_strlen(buf));
        level = MSG_INFO;  //CONN-EC-WIFI-LOG-01+
	...
	//CONN-FY-FP7372_EAPSIM_Integration-00--
		...
		if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED)
			...
			wpa_s->reassociate = 1;
            wpas_select_network_from_last_scan(wpa_s);  //CONN-EC-WIFI-ConnectSpeed-01+
			wpa_supplicant_req_scan(wpa_s, 0, 0);
		}
