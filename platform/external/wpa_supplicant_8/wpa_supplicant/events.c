			      ...
			      struct wpa_ssid *ssid)
{
    ...
    /*
        *   In Multi-scard mode, only if network have set pcsc, supplicant will 
        *   init scard every time when connect to AP   
        */
    if (ssid->eap.pcsc == NULL) {
		return 0;
    }
	if(wpa_s->scard != NULL) {
	...
	type = SCARD_TRY_BOTH;  //CONN-FY-FP7372_EAPSIM_Integration-00
#if defined(MULTI_SCARD)
	wpa_s->scard = scard_init(type, wpa_s->apdu_socket, ssid->eap.sim_slot);  //CONN-FY-DS_Support_EAPSIM-00
#else
    wpa_s->scard = scard_init(type, wpa_s->apdu_socket); //CONN-FY-FP7372_EAPSIM_Integration-00
#endif
	if (wpa_s->scard == NULL) {
	...
	if (wpa_s->reassociate ||
	    (os_memcmp(selected->bssid, wpa_s->bssid, ETH_ALEN) != 0 &&
	     ((wpa_s->wpa_state != WPA_ASSOCIATING &&
	      ...
	      0))) {
		if (wpa_supplicant_scard_init(wpa_s, ssid) != 0) {
            //CONN-FY-DS_Support_EAPSIM-00++
			wpa_s->current_ssid = ssid;  //Notify Android framework for disabling network if init fail 3 times
			wpa_msg( wpa_s, MSG_INFO, WPA_EVENT_EAP_FAILURE "EAP authentication failed");
			wpa_supplicant_set_state( wpa_s, WPA_DISCONNECTED);
			wpa_s->current_ssid = NULL;
			//CONN-FY-DS_Support_EAPSIM-00--
